...
bool
XSSFilter::StartFilterInternalScript(nsCString mScriptContent, ScriptLoadRequest* aRequest) {
  uint32_t paramLength = urlPostParams.Length();
  nsString paramName, paramValue;
  ...
  URLParams params = GetGETData(aRequest->mURI);
...
URLParams
XSSFilter::GetGETData(nsIURI* aURI) {
  URLParams params;
